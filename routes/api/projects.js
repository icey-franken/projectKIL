...
router.put('/edit/:projectId(\\d+)', routeHandler(async(req, res, next) => {
    ...
    console.log('line 95', destructions);
    if (!Array.isArray(supplies)) { supplies = supplies.split(','); }
    if (!Array.isArray(destructions)) { destructions = destructions.split(','); }
    ...
    Project.findByPk(projectId)
        .then((project) => {
            ...
            project.name = name;
            project.intro = intro;
            project.supplies = Array.from(supplies);
            project.destructions = Array.from(destructions);
            project.destructionsHeadings = Array.from(destructionsHeadings);
            project.update({
                    name: project.name,
                    intro: project.intro,
