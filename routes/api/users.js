...
const validateUserInfoFields = [
  check("countryId", "Please include a country.").exists({ checkNull: true }),
  check("aboutYouId", "Please include an about you.").exists({
    checkNull: true,
  }),
];
...
router.post(
  ...
  routeHandler(async (req, res) => {
    ...
    const tokens = cookies
      .split("; ")
      .filter((cookie) => cookie.slice(0, 6) === "token=")
